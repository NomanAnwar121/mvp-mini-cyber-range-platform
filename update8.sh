...
echo "======================================"
echo " Patching lab links + IDOR + XSS"
echo "======================================"
        ...
        <div class="challenge-box">
            ...
            <div class="button-group">
                ...
                {% elif challenge.id == 'xss' %}
                    <a class="btn" href="{{ url_for('lab_xss') }}?q=welcome">Open Reflected XSS</a>
                {% elif challenge.id == 'stored_xss' %}
                    <a class="btn" href="{{ url_for('lab_stored_xss') }}">Open Stored XSS</a>
                {% elif challenge.id == 'idor' %}
                    <a class="btn" href="{{ url_for('lab_idor') }}?id=1001">Open IDOR</a>
                {% endif %}
...
<div class="card">
    ...
    <p>This page simulates a vulnerable search application using the <code>q</code> URL parameter.</p>
    <p><strong>Example:</strong> <code>{{ example_url }}</code></p>
    <p><strong>Attempts so far:</strong> {{ attempts }}</p>

    <form method="GET" action="{{ url_for('lab_xss') }}" class="form">
        <input type="text" name="q" placeholder="Search products, tickets, docs..." value="{{ query }}">
...
<div class="card">
    ...
    <p><strong>Current q parameter:</strong> {{ query }}</p>
...
echo "[+] Writing templates/lab_stored_xss.html ..."
cat > templates/lab_stored_xss.html <<'EOF'
{% extends "base.html" %}
{% block content %}
<div class="card">
    <h1>Stored XSS Lab</h1>
    <p>This page simulates a vulnerable blog comment system that stores input and renders it unsafely.</p>
    <p><strong>Attempts so far:</strong> {{ attempts }}</p>
...
<div class="card">
    ...
    <div class="log-box">
        ...
        <div class="unsafe-box">{{ comment.content|safe }}</div>
    </div>
    {% endfor %}
</div>
...
echo "python3 app.py"
echo ""
echo "Expected lab links now:"
echo "/lab/idor?id=1001"
echo "/lab/xss?q=welcome"
echo "/lab/stored-xss"
