...
echo "======================================"
echo " Upgrade: Interactive Campaigns + IDOR + Admin + Adaptive Hints"
echo "======================================"
...
CHALLENGES = [
    {
        ...
        "simulation_steps": [
            ...
            "Probe: test single quote behavior",
            "Exploit: inject authentication bypass payload",
            "Access: gain unauthorized admin access",
            "Capture: recover the challenge flag"
        ],
        "hint1": "Try a classic authentication bypass payload in the password field.",
        "hint2": "Use username admin and try a condition that always evaluates true."
    },
    {
        "id": "xss",
        ...
        "points": 10,
        "simulation_steps": [
            "Recon: inspect reflected input",
            "Probe: test harmless HTML reflection",
            "Exploit: inject script payload",
            "Execution: trigger script handling",
            "Capture: reveal the challenge flag"
        ],
        "hint1": "Try a basic script payload.",
        "hint2": "A common starter payload uses alert(1) inside script tags."
    },
    {
        "id": "idor",
        "title": "IDOR Lab",
        "description": "Access another user's record by manipulating a direct object reference.",
        "difficulty": "Easy",
        "category": "Web Security",
        ...
        "points": 15,
        "simulation_steps": [
            "Recon: inspect numeric object identifiers",
            "Probe: modify the record identifier",
            "Exploit: access unauthorized resource",
            "Discovery: locate sensitive data in another user's record",
            "Capture: recover the challenge flag"
        ],
        "hint1": "Look at how the profile ID is used.",
        "hint2": "Try changing the record ID to access another user's data."
    }
]
        ...
        "steps": [
            {
                ...
                "instruction": "Review the available labs and identify likely vulnerable areas: login form, reflected search, and object IDs."
            },
            {
                "order": 2,
                "id": "sqli_step",
                ...
                "instruction": "Exploit the login flow and recover the SQLi flag."
            },
            {
                "order": 3,
                "id": "xss_step",
                ...
                "instruction": "Exploit the unsafe reflected input and recover the XSS flag."
            },
            {
                "order": 4,
                "id": "idor_step",
                ...
                "challenge_id": "idor",
                "technique": "T1190 - Exploit Public-Facing Application",
                "instruction": "Manipulate an object identifier and access another user's data."
            },
            {
                "order": 5,
                "id": "objective",
...
IDOR_RECORDS = {
    "1001": {"owner": "student", "notes": "Your own profile record. Nothing sensitive here."},
    "1002": {"owner": "analyst", "notes": "Internal note: routine audit account."},
    "1003": {"owner": "admin", "notes": "Sensitive admin record. Flag: FLAG{IDOR_DISCOVERED}"}
}
...
def lab_sqli():
    ...
    if request.method == "POST":
        ...
        if username_input == "admin" and password_input == "supersecret":
            ...
            "' OR '1'='1" in password_input or '" OR "1"="1' in password_input
        ):
...
def lab_xss():
    ...
    if request.method == "POST":
        ...
        if payload_lower in [p.lower() for p in valid_payloads]:
            executed = True
            flag = "FLAG{XSS_HUNTER}"
...
@app.route("/lab/idor", methods=["GET", "POST"])
def lab_idor():
    if "username" not in session:
        return redirect(url_for("index"))
    username = session["username"]

    record_id = request.values.get("record_id", "1001")
    ...
    if record:
        if record_id == "1003":
            flag = "FLAG{IDOR_DISCOVERED}"
            message = "Unauthorized admin record accessed."
            success = True
        else:
            message = f"Viewing record {record_id} owned by {record['owner']}."
    else:
...
<div class="card">
    ...
    <p><strong>Goal:</strong> inject a payload that triggers a reflected execution condition.</p>
    <p><strong>Attempts so far:</strong> {{ attempts }}</p>
...
<div class="card">
    <h1>IDOR Lab</h1>
    <p><strong>Goal:</strong> manipulate the record ID to access another user's sensitive data.</p>
    <p><strong>Attempts so far:</strong> {{ attempts }}</p>

    <form method="POST" class="form">
        <input type="text" name="record_id" placeholder="Enter record ID" value="{{ record_id }}" required>
...
echo "2) start campaign"
echo "3) complete recon step"
echo "4) solve SQLi, XSS, IDOR"
echo "5) submit flags in Tasks"
